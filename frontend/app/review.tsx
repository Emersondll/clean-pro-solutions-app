...
export default function ReviewScreen() {
  ...
  const { user } = useAuth();
  const params = useLocalSearchParams<{ reviewedId?: string; contractId?: string; contractorName?: string; serviceName?: string }>();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await api.post('/ratings', {
        reviewerId: user.id,
        reviewedId: params.reviewedId ?? 'unknown-contractor',
        contractId: params.contractId ?? 'unknown-contract',
        score: rating,
      ...
      const msg = error?.response?.data?.message ?? 'Erro ao enviar avaliação. Tente novamente.';
    ...
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <View style={styles.profCard}>
          <View style={styles.avatar}>
            <Text style={styles.avatarText}>
              {(params.contractorName ?? 'P').charAt(0).toUpperCase()}
            </Text>
          </View>
          <Text style={styles.profName}>{params.contractorName ?? 'Profissional'}</Text>
          <Text style={styles.serviceName}>{params.serviceName ?? 'Serviço de Limpeza'}</Text>
        </View>
        ...
        <Text style={styles.ratingText}>
          {rating === 0 ? 'Selecione uma nota' : RATING_LABELS[rating]}
        </Text>
      ...
      <View style={styles.footer}>
        <Button title={submitting ? 'Enviando...' : 'Enviar Avaliação'} onPress={handleSubmit} />
      </View>
...
const styles = StyleSheet.create({
  ...
  avatar: {
    ...
    alignItems: 'center', justifyContent: 'center', marginBottom: 12, ...theme.shadows.md,
  },
  ...
  commentInput: {
    ...
    borderRadius: 16, padding: 16, height: 120, fontSize: 15,
  },
  footer: {
